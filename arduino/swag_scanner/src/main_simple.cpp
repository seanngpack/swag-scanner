...
// these are int characteristics so .value() returns an int
  ...
  if (central)
  {
    ...
    while (central.connected())
    {
      ...
      if (rotateTableCharacteristic.written())
      {
        ...
        if (validValue(value))
        { 
          Serial.println("does it actually get here...?");
...
{
    Serial.print("rotatetable method received: ");
    Serial.println(degs);
  ...
  // Serial.print("next position at: ");
  // Serial.println(nextStepperPosition);
  ...
  {
    Serial.println(stepsToDeg(stepper.currentPosition()));
    stepper.runSpeed();
  }
